            ...
            for i in range(self.num_sensors):
                ...
                if (self.sensor_list[i].state == "Start"):
                    ...
                    if len(self.sensor_list[i].avg_arr) < self.sensor_list[i].max_length:
                        self.adc[i].avg_arr = self.adc[i].avg_arr + (value,) #add a value
                    else:
                        #shift every array elem down 1 and remove oldest
                        #put in new elem
                        self.adc[i].avg_arr = self.adc[i].avg_arr[1:3] + (value,) #remove one and add
                    self.sensor_list[i].avg = sensors_c.avg(self.sensor_list[i].avg_arr)
